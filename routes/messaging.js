...
router.post("/send", (req, res) => {
    ...
    .then(returned => {
        ...
        res.send({
            success: false,
            error: err + "could not find member ID for sender",
        });
    });



//   If the conversation is not there already, insert it
    db.none('INSERT INTO Conversations (chatId, name) VALUES ($1, $2)' , [memberID, "Global Chat2"] )
    .then({}).catch((err) => {
        res.send({
            success: false,
            error: err + " Could not insert conversation into DB",
        })
    });
